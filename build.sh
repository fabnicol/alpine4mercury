...
then
    ...
    then
        echo "alpine4mercury build script version $(cat VERSION)"
...
if docker build --squash --file ${DOCKERFILE1} --tag alpine:mercury${REVISION} .
then
    echo "Docker image was built as alpine:mercury${REVISION}"
else
...
if docker save alpine:mercury${REVISION} -o alpine4mercury.tar
then
  ...
  then
      ...
      if [ "${COMPRESS_XZ}" = "true" ]
      then
        if  xz -9 -k -f alpine4mercury.tar
        then
          echo b2sum: $(b2sum alpine4mercury.tar.xz) >> SUMS
          echo sha512sum: $(sha512sum alpine4mercury.tar.xz) >> SUMS
	else
      ...
      if [ "${COMPRESS_GZ}" = "true" ]
      then
        if gzip -f -k alpine4mercury.tar
	then
	  echo b2sum: $(b2sum alpine4mercury.tar.gz) >> SUMS
	  echo sha512sum: $(sha512sum alpine4mercury.tar.gz) >> SUMS
	else
